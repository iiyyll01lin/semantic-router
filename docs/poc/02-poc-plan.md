...
- gfx1151（RDNA 3.5）仍不是任意 vLLM/ROCm 映像都可假設支援的通用目標；舊 `rocm/vllm-dev` 映像曾以 `invalid device function` 失敗。不過 2026-07-22 已在 Strix Halo 上實測 pinned official vLLM v0.25.1 + `ROCM_ATTN` + BF16 Qwen2.5-7B 可執行，48/48 個 16K/32K batching/APC cells 都有 checkpoint，且無 OOM/restart。這是**特定映像／模型／backend 的實驗性支援邊界**，不是 blanket support；BF16 結果也不能與 GGUF Ollama/llama.cpp 直接比速度。穩健預設仍可用 llama.cpp ROCm、Ollama 或 AMD Lemonade Server。router 本身是後端無關的，任何 OpenAI 相容 endpoint 都能透過 `backend_refs` 接上。
  gfx1151 (RDNA 3.5) is still not a blanket target that arbitrary vLLM/ROCm images can assume; an older `rocm/vllm-dev` image failed with `invalid device function`. However, a pinned official vLLM v0.25.1 + `ROCM_ATTN` + BF16 Qwen2.5-7B stack was measured working on Strix Halo on 2026-07-22: all 48 planned 16K/32K batching/APC cells checkpointed without OOM or restart. This is an **experimental support boundary for that exact image/model/backend**, not blanket support, and its BF16 numbers are not directly comparable with GGUF Ollama/llama.cpp. llama.cpp ROCm, Ollama, and AMD Lemonade Server remain conservative defaults. The router is backend-agnostic, so any OpenAI-compatible endpoint connects via `backend_refs`.
