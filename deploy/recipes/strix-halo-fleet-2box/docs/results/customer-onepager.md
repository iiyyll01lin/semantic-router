...
| Halo-B peak (96 GiB VRAM, headless) | **`gpt-oss:120b` (120B MoE)** | **~37 tok/s** | VRAM-resident; 70B-Q8 (~70 GiB) also resident; largest measured 141B MoE at **~94.6 GiB** (essentially the full carveout) |
...
**Best config vs naive default (measured end-to-end):** configured correctly (VRAM-resident + `OLLAMA_NUM_PARALLEL=8` + cache + llama.cpp), the same box runs the **big model VRAM-resident (36.5 tok/s) instead of CPU-offloaded**, at **~2.8× the concurrent throughput** and **~10× the energy efficiency per token** of a naive default — the gain is configuration, not hardware.
...
- **vs a discrete GPU:** one integrated box's **unified memory replaces a >40 GB GPU card**; the 120B MoE is even **more power-efficient per token than a dense 32B — 0.382 vs 0.093 tok/s/W** (7B is 0.41) — a far larger model with no bigger power bill.

## Recommended settings

- Semantic-cache threshold **0.92** — max coverage with zero false hits.
- **`OLLAMA_NUM_PARALLEL=8`** → aggregate **~128 tok/s** (~3× the serialized default; throughput knee at 8 concurrent streams).
