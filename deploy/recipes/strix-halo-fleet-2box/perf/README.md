...
| [`maxmodel-sweep.sh`](maxmodel-sweep.sh) | **Ascending "max model under topology" ceiling sweep**, run LOCALLY with the vllm-sr stack left **co-resident**: per rung it samples the unified-memory split (VRAM carveout + GTT overflow + host RAM) during a real decode and classifies `usable` / `unusable(slow-spill)` / `load-fail` **and** `vram-fit` vs `gtt-spill`. `NUM_CTX` forces a footprint past the carveout to probe the spill boundary. Measured write-up: [Halo-B max-model note](../docs/halo-b-maxmodel.md). |
