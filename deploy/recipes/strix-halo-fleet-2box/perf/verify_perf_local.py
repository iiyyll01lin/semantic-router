...
def verify_tokrate(base, tmp):
    rc = tokrate_probe.main(
        [
            ...
            "8",
            "--out",
            os.path.join(tmp, "oll.json"),
        ]
    )
    oll = json.load(open(os.path.join(tmp, "oll.json"), encoding="utf-8"))
    check(
        "1. ollama exit 0 + decode_tps==120 + prefill==80",
        rc == 0
        and abs(oll["aggregate"]["decode_tps_median"] - 120.0) < 1e-6
    ...
    rc = tokrate_probe.main(
        [
            ...
            "8",
            "--out",
            os.path.join(tmp, "oai.json"),
        ]
    )
    oai = json.load(open(os.path.join(tmp, "oai.json"), encoding="utf-8"))
    check(
        "2. openai exit 0 + completion_tokens==50 + decode_tps>0 + ttft set",
        rc == 0
        and oai["runs_detail"][0]["completion_tokens"] == 50
    ...
    check(
        "4. unreachable backend -> ok_runs==0 + exit 1",
...
def verify_sampler(tmp):
        ...
        for used in (10, 20, 30):
            fh.write(
                json.dumps(
                    {
                        "t": 1,
                        "gpu": {"vram_used_b": used, "gtt_used_b": used * 2},
                        "host": {
                            "mem_total_b": 1000,
                            "mem_available_b": 100,
                            "mem_used_b": 900,
    ...
    check(
        "5. sampler parse + summarize (peak vram 30 / gtt 60 / mean 20) + snapshot ok",
        abs(resource_sampler._parse_bytes("1.5GiB") - 1.5 * 1024**3) < 1
        and summ["peak_vram_used_b"] == 30
        and summ["gpu"]["gtt_used_b"]["max"] == 60
...
def verify_repoint(tmp):
    ...
    rc_missing = repoint_backend.main(
        ["--config", cfg, "--alias", "no/such", "--endpoint", "x:1", "--model", "y"]
    )
    check(
        "6. repoint in-place (inode kept), only target block, missing alias -> exit 1",
        rc == 0
        and "endpoint: llama-server:8080" in txt
        and "vllm: qwen2.5-7b" in txt
        and txt.count("endpoint: ollama:11434") == 1
        and "vllm: qwen3:14b" in txt
        and os.stat(cfg).st_ino == ino
        and rc_missing == 1,
    )
...
def verify_aggregate(tmp):
    ...
    json.dump(
        {
            "box": "halo-a",
            "unified_mem_total_b": 128 * 1024**3,
            "stack_footprint": {"stack_container_mem_total_b": 4 * 1024**3},
            "tiers": [
                {
                    "tag": "qwen2.5:7b",
                    "throughput_drop_pct_contention": 10.0,
                    "throughput_drop_pct_end_to_end": 18.0,
                }
            ],
            "max_usable_tag": "qwen2.5:32b",
            "first_unusable_tag": "llama3.1:70b",
        },
    ...
    json.dump(
        {
            ...
            "tiers": [
                {
                    "tag": "qwen2.5:7b",
                    "throughput_drop_pct_contention": 20.0,
                    "throughput_drop_pct_end_to_end": 30.0,
                }
            ],
            "max_usable_tag": "qwen2.5:14b",
            "first_unusable_tag": "qwen2.5:32b",
        },
    ...
    json.dump(
        {
            "box": "halo-a",
            "common_base_model": "qwen2.5-7b",
            "servers": [
                {
                    "server": "ollama",
                    "status": "measured",
                    "quant": "Q4_0",
                    "direct_decode_tps": 40.0,
                    "direct_ttft_ms": 100,
                },
                {
                    "server": "llamacpp",
                    "status": "measured",
                    "quant": "Q4_K_M",
                    "direct_decode_tps": 55.0,
                    "direct_ttft_ms": 80,
                    "decode_tps_vs_ollama_pct": 37.5,
                },
                {
                    "server": "vllm",
                    "status": "skipped",
                    "reason": "bring-up failed",
                    "quant": "fp16",
                },
            ],
        },
    ...
    check(
        "7. fleet aggregate (mean drop 15, worst-box max-usable 14b, fastest llamacpp, skip kept) + md",
        m["overhead"]["boxes"] == ["halo-a", "halo-b"]
        and abs(m["overhead"]["per_tier_drop"][0]["mean_drop_pct_contention"] - 15.0)
        < 1e-6
        and m["overhead"]["fleet_max_usable_tag"] == "qwen2.5:14b"
        and m["servers"]["per_box"]["halo-a"]["fastest_server"] == "llamacpp"
        and any(
            s["status"] == "skipped"
            for s in m["servers"]["per_box"]["halo-a"]["servers"]
        )
        and "Test 1" in md
        and "Test 2" in md,
    )
...
def _matrix_cell(
    ...
    return {
        ...
        "reason": None,
        "c1": {
            "decode_tps_median": c1_tps,
            "aggregate_decode_tps": c1_tps,
            "ttft_ms_p50": ttft_p50,
            "ttft_ms_p95": ttft_p50,
            "ok_runs": 3,
        },
        "c8": {
            "decode_tps_median": c1_tps,
            "aggregate_decode_tps": agg_c8,
            "ttft_ms_p95": ttft_p95,
            "ttft_ms_mean": ttft_p95,
            "ok_runs": 8,
        },
...
def verify_matrix_logic():
    ...
    probe = {
        "aggregate": {
            "decode_tps_median": 30.0,
            "aggregate_decode_tps": 120.0,
            "ttft_ms_mean": 150.0,
            "ttft_ms_p95": 300.0,
            "success_rate": 1.0,
            "ok_runs": 3,
        ...
        "runs_detail": [
            {"ok": True, "ttft_ms": 100.0},
            {"ok": True, "ttft_ms": 200.0},
    ...
    st_unus, _ = bestcfg_matrix.classify(
        "ok", {"ok_runs": 2, "decode_tps_median": 1.5}, 3.0
    )
    st_ok, _ = bestcfg_matrix.classify(
        "ok", {"ok_runs": 2, "decode_tps_median": 36.0}, 3.0
    )
    st_empty, _ = bestcfg_matrix.classify(
        "ok", {"ok_runs": 0, "decode_tps_median": None}, 3.0
    )
    check(
        "8. probe reduce (p50=150, agg=120) + classify fail/unusable/loaded/empty",
        red["ttft_ms_p50"] == 150.0
        and red["aggregate_decode_tps"] == 120.0
        and st_fail == "load-fail"
        and st_unus == "unusable"
        and st_ok == "loaded"
        and st_empty == "load-fail",
    )
    ...
    check(
        "9. winner rule: c8-agg primary + gate, per-server winners, tok/W tie-break, gate fallback",
        sc["winner_cell_id"] == "ollama-resident-p8"
        and sc["winner_ttft_gate_met"] is True
...
def verify_matrix_assemble(tmp):
    ...
    def _probe(agg, decode, ttft95, conc):
        return {
            ...
            "runs_detail": [{"ok": True, "ttft_ms": ttft95}],
            "shape": {"concurrency": conc},
        }
    ...
    _write(
        "ollama-resident-p8.load.json",
        {
            "cell_id": "ollama-resident-p8",
            "server": "ollama",
            "residency": "resident",
            "num_parallel": 8,
    ...
    _write(
        "ollama-resident-p8.res.json",
        {
            "peak_vram_used_b": 66 * 1024**3,
            "gpu": {"vram_used_b": {"max": 66 * 1024**3}},
            "host": {"mem_used_b": {"max": 6 * 1024**3}},
        },
    )
    _write(
        "ollama-resident-p8.power.json",
        {
            "api": "ollama",
            "model": "gpt-oss:120b-vram",
            "idle_w": 10.0,
            ...
            "load_w_peak": 110.0,
            "decode_tps": 36.5,
            "tok_per_watt_load": 0.365,
            "tok_per_watt_net_idle": 0.405,
        },
    )
    # A skipped llamacpp cell (load probe failed) -- must survive assembly.
    _write(
        "llamacpp-resident-p8.load.json",
        {
            ...
            "model": "gpt-oss-120b",
            "load_result": "skipped",
            "reason": "llama.cpp MXFP4 120B load probe failed",
        },
    )
    ...
    _write(
        "cache-ollama.json",
        {
            ...
            "cell_id": "ollama-resident-p8",
            "ttft_miss_ms": 1300.0,
            "ttft_hit_exact_ms": 2.0,
            "ttft_hit_semantic_ms": 800.0,
            "ttft_saved_ms": 1298.0,
            "exact_hit_rate": 1.0,
    ...
    check(
        "10. assemble rollup: winner ollama-resident-p8, skipped cell kept, overlay + conclusion",
...
def verify_matrix_cache_overlay(tmp):
    ...
    try:
        ...
        srv.shutdown()
    check(
        "11. cache overlay: exact-repeat hit registered, hit TTFT <= miss TTFT",
        ov["exact_hit_rate"] == 1.0
        and ov["ttft_miss_ms"] is not None
        and ov["ttft_hit_exact_ms"] is not None
