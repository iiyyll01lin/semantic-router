...
def aggregate_overhead(files):
    ...
    max_usable = [
        rep.get("max_usable_tag") for rep in boxes.values() if rep.get("max_usable_tag")
    ]
    footprints = [
        rep.get("stack_footprint", {}).get("stack_container_mem_total_b")
        for rep in boxes.values()
    ]
    return {
        "boxes": sorted(boxes),
        "per_box": {
            b: {
                ...
                "stack_footprint_gib": _gib(
                    r.get("stack_footprint", {}).get("stack_container_mem_total_b")
                ),
...
def aggregate_servers(files):
    ...
    for path in files:
        ...
        measured = [
            r
            for r in rows
            if r.get("status") == "measured" and r.get("direct_decode_tps")
        ]
        fastest = (
            max(measured, key=lambda r: r["direct_decode_tps"]) if measured else None
        )
        boxes[box] = {
    ...
    fastest_votes = [
        b["fastest_server"] for b in boxes.values() if b.get("fastest_server")
    ]
    consensus = None
    ...
    return {
        "boxes": sorted(boxes),
        "per_box": boxes,
        "fleet_fastest_consensus": consensus,
    }
...
def to_markdown(m):
    ...
    if ov:
        ...
        out.append("")
        out.append(
            "Fleet-safe max usable model: **%s**  ·  mean stack RAM footprint: **%s GiB**"
        ...
        for b, r in sorted(ov["per_box"].items()):
            out.append(
                ...
                % (
                    b,
                    r["unified_mem_gib"],
                    r["stack_footprint_gib"],
                    r["max_usable_tag"],
                    r["first_unusable_tag"],
                )
            )
        out.append("")
        out.append(
        ...
        for t in ov["per_tier_drop"]:
            out.append(
                ...
                % (
                    ...
                    (
                        "-"
                        if t["mean_drop_pct_contention"] is None
                        else "%.1f" % t["mean_drop_pct_contention"]
                    ),
                    (
                        "-"
                        if t["mean_drop_pct_end_to_end"] is None
                        else "%.1f" % t["mean_drop_pct_end_to_end"]
                    ),
                    (
                        "-"
                        if t.get("mean_baseline_ttft_ms") is None
                        else "%.0f" % t["mean_baseline_ttft_ms"]
                    ),
                    (
                        "-"
                        if t.get("mean_router_ttft_ms") is None
                        else "%.0f" % t["mean_router_ttft_ms"]
                    ),
                )
            )
        out.append("")
    ...
    if sv:
        ...
        if sv.get("fleet_fastest_consensus"):
            out.append(
                "Fleet consensus fastest server: **%s**" % sv["fleet_fastest_consensus"]
            )
            out.append("")
        for b, rec in sorted(sv["per_box"].items()):
            out.append(
                "### %s (base %s, fastest: %s)"
                % (b, rec.get("common_base_model"), rec.get("fastest_server"))
            )
            out.append("")
            out.append(
            ...
            for r in rec["servers"]:
                out.append(
                    ...
                    % (
                        ...
                        (
                            "-"
                            if r.get("direct_decode_tps") is None
                            else "%.1f" % r["direct_decode_tps"]
                        ),
                        (
                            "-"
                            if r.get("direct_ttft_ms") is None
                            else "%.0f" % r["direct_ttft_ms"]
                        ),
                        (
                            "-"
                            if r.get("decode_tps_vs_ollama_pct") is None
                            else "%+.1f%%" % r["decode_tps_vs_ollama_pct"]
                        ),
                        (
                            "-"
                            if r.get("router_overhead_pct") is None
                            else "%.1f" % r["router_overhead_pct"]
                        ),
                        r.get("quant") or "-",
                    )
                )
...
def main(argv=None):
    ...
    p.add_argument(
        "--bundle", required=True, help="run bundle directory with per-box perf JSON"
    )
    p.add_argument(
        "--out",
        default="",
        help="perf-metrics.json path (default: <bundle>/perf-metrics.json)",
    )
    p.add_argument(
        "--summary",
        default="",
        help="perf-summary.md path (default: <bundle>/perf-summary.md)",
    )
