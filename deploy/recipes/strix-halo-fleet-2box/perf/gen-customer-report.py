...
def main():
    ...
    ap.add_argument(
        "--hw-usd", type=float, default=2500.0, help="one-off box hardware cost"
    )
    a = ap.parse_args()
    B = a.bundle
    ...
    stack = (overhead.get("stack_footprint", {}) or {}).get(
        "stack_container_mem_total_b"
    )
    stack_gib = round(stack / 1024**3, 1) if stack else None
    tiers = overhead.get("tiers", []) or []
    d_ttft = next(
        (t.get("baseline_ttft_ms") for t in tiers if t.get("baseline_ttft_ms")), None
    )
    r_ttft = next(
        (
            t.get("colocated_router_ttft_ms")
            for t in tiers
            if t.get("colocated_router_ttft_ms")
        ),
        None,
    )
    ...
    o.append("")
    o.append(
        "_Box: %s · unified memory: %s GiB · vllm-sr stack footprint: %s GiB_"
    ...
    o.append("")
    o.append(
        "- **It runs today:** router + backend share one box; the router costs ~%s GiB "
        "of memory and near-zero decode throughput." % fmt(stack_gib)
    )
    o.append(
        "- **The real cost is first-token latency:** direct ~%s ms -> through the router "
        "~%s ms (**+%s s**), which a semantic-cache hit removes entirely."
        % (fmt(d_ttft, "%.0f"), fmt(r_ttft, "%.0f"), fmt(tax, "%.1f"))
    )
    o.append(
        "- **Feasibility is memory-bound:** the biggest model a box serves = unified "
    ...
    o.append("")
    o.append(
        "Fleet-safe max usable model: **%s**."
        % (metrics.get("overhead", {}) or {}).get(
            "fleet_max_usable_tag", overhead.get("max_usable_tag", "-")
        )
    )
    o.append("")
    if maxmodel.get("results"):
        ...
        for r in maxmodel["results"]:
            ...
            o.append(
                ...
                % (
                    r.get("tag"),
                    fmt(r.get("est_footprint_gib")),
                    fmt(r.get("projected_pct")),
                    ran_on,
                    fmt(tps, "%.1f"),
                    verdict_disp,
                )
        ...
        o.append("")
        o.append(
            "_Models whose projected memory use exceeds %s%% of one box are offloaded to Halo-B; "
    ...
    if unified_gib:
        ...
        o.append(
            "### Quantization decides the ceiling (%s GiB usable after stack + 10%% reserve)"
            % usable
        )
        o.append("")
        ...
        o.append("")
        o.append(
            "_Q4 roughly **doubles** the largest model vs fp16 on the same hardware — the "
    ...
    o.append("")
    o.append(
        "The router adds ~%s s to first-token latency (classification + embedding + routing). "
        "A **semantic-cache hit** skips that pipeline and the model call entirely:"
        % fmt(tax, "%.1f")
    )
    o.append("")
    ...
    if csvp and os.path.exists(csvp):
        ...
        if len(rows) > 1:
            ...
            try:
                ...
                if ok:
                    o.append(
                        "_Recommended threshold: **%s** — the lowest that never serves a wrong "
                        "cached answer (false-hit = 0), maximising coverage._"
                        % ok[0][ti]
                    )
                    o.append("")
            except Exception:
    ...
    for p in sorted(glob.glob(os.path.join(B, "conc-c*-*.json"))):
        ...
        concs.append(
            (
                ag.get("concurrency"),
                ag.get("aggregate_decode_tps"),
                ag.get("ttft_ms_p95"),
            )
        )
    if concs:
        base = next(
            (tps for c, tps, _ in sorted(concs, key=lambda x: x[0] or 0) if tps), None
        )
        ...
        for c, tps, p95 in sorted(concs, key=lambda x: x[0] or 0):
            ...
            o.append(
                ...
                % (
                    c,
                    fmt(tps, "%.0f"),
                    ("%.2fx" % scal) if scal is not None else "-",
                    fmt(p95, "%.0f"),
                )
            )
        o.append("")
        o.append(
            "_Default Ollama serves one stream at a time: total throughput stays flat while "
        ...
        for r in rows:
            o.append(
                ...
                % (
                    r.get("server"),
                    r.get("quant", "-"),
                    fmt(r.get("direct_decode_tps"), "%.1f"),
                    fmt(r.get("direct_ttft_ms"), "%.0f"),
                    r.get("status", "-"),
                )
            )
        o.append("")
        o.append(
            "_Quantization differs per server, so decode-rate deltas are **not** apples-to-apples "
    ...
    rate = next(
        (
            t.get("baseline_decode_tps")
            for t in tiers
            if "7b" in (t.get("tag") or "").lower()
        ),
        None,
    )
    o.append(
        "**(a) vs cloud API.** After the one-off box cost (~$%.0f), local tokens are ~$0 marginal. "
        "At $%.2f / 1M output tokens, the box pays for itself after ~%s output tokens."
        % (
            a.hw_usd,
            a.cloud_usd_per_1m,
            human_tokens(
                a.hw_usd / (a.cloud_usd_per_1m / 1e6) if a.cloud_usd_per_1m else None
            ),
        )
    )
    o.append("")
    o.append(
        "**(b) vs no routing.** vllm-sr sends easy queries to a small model instead of always the "
    ...
    o.append("")
    o.append(
        "**(c) vs a discrete GPU.** Strix Halo's %s GiB unified memory holds a 32B that would need a "
        ">40 GB discrete card; one integrated box replaces a GPU-server tier (lower capex + power)."
        % fmt(unified_gib)
    )
    o.append("")
    o.append(
        "_Marginal per-token cost is ~$0 locally; the levers that make it genuinely cheaper: "
