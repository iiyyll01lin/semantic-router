...
Decode rate comes from Ollama's server-side ``eval_count`` / ``eval_duration``
(authoritative, no client-side timing noise), the same source as
`tokrate_probe.py`. Stdlib only (urllib/json/subprocess/threading) so it runs on
a bare box with no pip installs, matching the rest of this recipe. It NEVER
serves, deploys, or mutates config -- it only reads power and sends requests.
...
Exit code is 0 when the decode + power sampling produced a rate, else 1 (so a
caller can skip a box/model that never answered).
"""
...
def main(argv=None):
    ...
    p.add_argument("--model", required=True, help="ollama model tag, e.g. qwen2.5:7b")
    ...
    p.add_argument("--no-mmap", action="store_true",
                   help="send options.use_mmap=false (much faster load for 120B w/ CPU tensor overrides)")
    ...
    use_mmap = False if args.no_mmap else None
    ...
    print("== phase 2: warmup (load %s) ==" % args.model, flush=True)
    t_warm = time.perf_counter()
    try:
        ec, _, _ = ollama_generate(args.base_url, args.model, prompt, 16, args.think,
                                   args.num_ctx, use_mmap, args.keep_alive)
    except (urllib.error.URLError, urllib.error.HTTPError, OSError, ValueError) as exc:
        print("   warmup failed: %s: %s" % (type(exc).__name__, exc), file=sys.stderr)
        return 1
    print("   warmup done in %.1fs (eval_count=%s)"
          % (time.perf_counter() - t_warm, ec), flush=True)
    ...
    for i in range(max(1, args.runs)):
        try:
            ec, ed, wall = ollama_generate(args.base_url, args.model, prompt, args.max_tokens,
        ...
        runs.append({"eval_count": ec, "eval_duration_ns": ed, "wall_s": wall, "decode_tps": tps})
        print("   run %d: eval_count=%s decode_tps=%.2f wall=%.1fs"
              % (i + 1, ec, tps or -1, wall), flush=True)
    ...
    total_tokens = sum(r["eval_count"] or 0 for r in runs)
    total_decode_s = sum((r["eval_duration_ns"] or 0) for r in runs) / 1e9
    ...
    out = {
        ...
        "shape": {
            ...
            "use_mmap": (False if args.no_mmap else "default"),
    ...
    if args.out:
        with open(args.out, "w", encoding="utf-8") as fh:
            fh.write(text + "\n")
    print(text)
    print("\n== SUMMARY: idle=%.1fW load=%.1fW tok/s=%.2f tok/s-per-W=%.4f =="
          % (summ["idle_w_mean"] or -1, summ["load_w_mean"] or -1,
             summ["decode_tps_median"] or -1, summ["tok_per_watt_load"] or -1))
    return 0 if summ["decode_tps_median"] else 1
