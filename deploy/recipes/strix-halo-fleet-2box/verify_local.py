    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            if self.path == "/config/loaded-hash":
                return self._json(200, {"hash": state.r.loaded_hash(), "source": "loaded"})
...
def _make_mtls_certs(dirpath: str):
    ...
    if not openssl:
        return None
    p = {name: os.path.join(dirpath, name + ".pem")
         for name in ("ca-cert", "ca-key", "server-cert", "server-key",
    ...
    def _leaf(cn: str, key: str, csr: str, cert: str, ext: str) -> None:
        _run("req", "-newkey", "rsa:2048", "-nodes", "-subj", "/CN=" + cn,
             "-keyout", key, "-out", csr)
        _run("x509", "-req", "-in", csr, "-out", cert, "-CA", p["ca-cert"],
             "-CAkey", p["ca-key"], "-CAcreateserial", "-CAserial", srl,
             "-days", "2", "-extfile", ext)

    try:
        _run("req", "-x509", "-newkey", "rsa:2048", "-nodes", "-days", "2",
             "-keyout", p["ca-key"], "-out", p["ca-cert"],
             "-subj", "/CN=fleet-verify-ca",
        ...
        return None
    return {"ca": p["ca-cert"], "server_cert": p["server-cert"],
            "server_key": p["server-key"], "client_cert": p["client-cert"],
...
def _standby_sync(src_ccp_dir: str, dst_ccp_dir: str, fleet_state_dir: str) -> bool:
    ...
    try:
        ...
        for name in os.listdir(src_desired):
            shutil.copy2(os.path.join(src_desired, name),
                         os.path.join(dst_desired, name))
...
def main() -> int:
    ...
    try:
        ...
        record("vendored Ed25519 matches RFC 8032 test vectors (interop)", False, repr(exc))
    seed_hex, pub_hex = fleet_lib.ed25519_keygen()
    ed_verifier = fleet_lib.ed25519_verifier(pub_hex)
    ed_bundle = fleet_lib.build_bundle(fleet_lib.ed25519_signer(seed_hex), "v1", "ed-config\n")
    ok_good, _r = fleet_lib.verify_bundle(ed_verifier, ed_bundle)
    tampered_ed = dict(ed_bundle); tampered_ed["config"] = "ed-config-TAMPERED\n"
    ok_tamper, _r = fleet_lib.verify_bundle(ed_verifier, tampered_ed)
    _seed2, other_pub = fleet_lib.ed25519_keygen()
    ok_forge, _r = fleet_lib.verify_bundle(fleet_lib.ed25519_verifier(other_pub), ed_bundle)
    ...
    record("Ed25519 sign/verify + tamper/forge/HMAC-downgrade all rejected",
    ...
    serve(ThreadingHTTPServer(("127.0.0.1", stale_port), _static_bundle_handler(stale_bundle)))
    time.sleep(0.2)
    ...
    ad_cfg = fleet_agent.AgentConfig(
        ccp_url="http://127.0.0.1:%d" % stale_port, router_api=boxes[0]["acfg"].router_api,
        config_file=boxes[0]["cfg_path"], signing_key=SIGNING_KEY, token=TOKEN,
        box_id="downgrade-box", poll_interval=0.05, apply_timeout=1.0)
    before_ad = boxes[0]["rstate"].active_hash()
    ad_res = fleet_agent.reconcile_once(ad_cfg, ad_state)
    after_ad = boxes[0]["rstate"].active_hash()
    record("anti-downgrade rejects a stale validly-signed bundle (config unchanged)",
           ad_res.get("result") == "rejected" and "anti-downgrade" in ad_res.get("reason", "")
           and before_ad == after_ad, ad_res.get("reason", ""))
    ...
    rb_cfg = fleet_agent.AgentConfig(
        ccp_url=ccp_url, router_api="http://127.0.0.1:%d" % rb_port, config_file=rb_path,
        signing_key=SIGNING_KEY, token=TOKEN, box_id="rollback-box", poll_interval=0.05,
        apply_timeout=1.0, health_path="/healthz", apply_backoff=30.0)
    ...
    with open(rb_path, "r", encoding="utf-8") as fh:
        on_disk = fh.read()
    record("auto-rollback on unloadable apply (invalid config -> loaded-hash mismatch, restores .bak, reports rolled_back)",
           rb_res.get("result") == "rolled_back" and "loaded-hash" in rb_res.get("reason", "")
    ...
    record("backoff after rollback (bad version not rewritten next cycle)",
           rb_res2.get("result") == "apply_backoff", rb_res2.get("reason", ""))
    ...
    d1.record_status({"box_id": "halo-a", "version": v3, "hash": "h",
                      "result": "applied", "apply_seconds": 0.5})
    d2 = ccp_server.CCPState(SIGNING_KEY, TOKEN, dur_dir)  # brand-new state = "restart"
    restored = (d2.version == v3 and d2.config_text == "dur-c\n" and d2._version_num == 3
                and d2._audit_total >= 1 and "halo-a" in d2.boxes)
    record("CCP durability: desired+version+audit restored across restart", restored,
           "restored version=%s (want %s)" % (d2.version, v3))
    next_v = d2.set_desired("dur-d\n")
    record("CCP durability: version counter persists (no collision after restart)",
           next_v == "v4", "next version=%s (want v4)" % next_v)
    ...
    if not certs:
        record("mTLS: client presenting FLEET_TLS_CLIENT_CERT/KEY is accepted (C1)",
               False, "openssl unavailable to mint certs")
        record("mTLS: certless client is rejected at the TLS handshake (C1)",
               False, "openssl unavailable to mint certs")
    else:
        ...
        ctx_ok = fleet_lib.client_ssl_context(env={
        ...
        record("mTLS: client presenting FLEET_TLS_CLIENT_CERT/KEY is accepted (C1)",
               ok_status == 200 and scheme == "https", "status=%s" % (ok_status,))
    ...
    sb_state.record_status({"box_id": "halo-a", "version": sb_v, "hash": sb_hash,
                            "result": "applied", "apply_seconds": 0.4})
    sb_state.record_status({"box_id": "halo-b", "version": sb_v, "hash": sb_hash,
                            "result": "applied", "apply_seconds": 0.6})
    active_total = sb_state._audit_total

    sb_standby = os.path.join(workdir, "standby-copy", "ccp")
    synced = _standby_sync(sb_active, sb_standby, os.path.join(workdir, "standby-fleet"))
    replicated_ok = (synced
                     and os.path.isdir(os.path.join(sb_standby, "desired"))
                     and os.path.isfile(os.path.join(sb_standby, "audit.log")))
    record("warm-standby sync replicates desired/ + audit.log (C2, ccp-standby-sync.sh)",
           replicated_ok, "dest=%s" % sb_standby)
    ...
    restore_ok = (
        ...
        and restored_b == sb_v
        and sb_next == "v4")
    record("warm-standby restore: fresh CCPState = latest version+config+audit (C2)",
           restore_ok, "restored=%s audit=%d next=%s"
           % (restored_version, restored_total, sb_next))
